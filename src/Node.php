...
class Node {
    ...
    const NOT_FIXED_SIZE = 20;

    protected int $type = self::FIXED_SIZE;
    ...
    public function parse(string $pathStr, array $rules, array $messages = []): void
    {
        ...
        if($firstSegment != '*' && !key_exists($firstSegment, $this->nested)){
            $this->nested[$firstSegment] = new self();
        }
        $tail = join('.', $pathSegments);
        if($firstSegment == '*'){
            $this->type = self::NOT_FIXED_SIZE;
            $firstSegment = array_shift($pathSegments);
            if($firstSegment && !key_exists($firstSegment, $this->nested)){
                $this->nested[$firstSegment] = new self();
            }
            if(!$firstSegment){
                $this->setRules($rules, $messages);
            }
            $tail = join('.', $pathSegments);
        }
        if($tail && key_exists($firstSegment, $this->nested)){
            $this->nested[$firstSegment]->parse($tail, $rules, $messages);
        }
        if(!$tail && key_exists($firstSegment, $this->nested)){
            $this->nested[$firstSegment]->setRules($rules, $messages);
        }
    }
    ...
    {
        $result = true;
        foreach($this->nested as $key=>$node){
            $val = $data[$key] ?? null;
    ...
    public function isValid($val, string $key, array &$errors): bool
    {
        $result = true;
        $e = [];
        foreach($this->rules as $rule){
            ...
            if(!$this->nested && $this->type == self::FIXED_SIZE){
                $this->cleanData = $val;
            }
            if(!$this->nested && $this->type == self::NOT_FIXED_SIZE){
                $this->cleanData[$key] = $val;
            }
        }
        $errors[$key] = $e;
